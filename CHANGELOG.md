...
- **Formal plugin contract + `coworker plugins` meta-command.** Optional plugins are now discovered automatically from the `coworker.plugins` package: a module that publishes a `MANIFEST` (`PluginManifest`: name, summary, version, optional `requires` binary) plus `register()`/`dispatch()` callables — and an optional `install()` lifecycle hook — is picked up with no edit to `cli.py`. New commands: `coworker plugins list` (text or `--format json`, with a PATH-presence status for plugins that wrap an external binary) and `coworker plugins install <name>` (runs the plugin's install hook, which prints instructions only — coworker never executes a network installer). The bundled `rtk` plugin adopts the contract; `coworker rtk …` is unchanged. Discovery is defensive: a plugin that fails to import is skipped with a warning and never takes the CLI down. Contract and authoring guide: [`docs/plugins.md`](docs/plugins.md).
- **`coworker rtk economics` — resilient combined spending-vs-savings view.** The upstream `rtk cc-economics` subcommand crashes against the current `ccusage` JSON schema (ccusage renamed each monthly row's key from `month` to `period`; RTK's parser hard-requires `month`). The new command rebuilds the consolidated «spending (ccusage) vs savings (RTK)» view natively from `ccusage monthly --json` and `rtk gain --format json`, tolerant to the `month`→`period` rename (and the older `month`/`date` schema). It is fail-soft — if either data source is unavailable the other half still prints, exiting `0` when at least one half rendered and `1` only when both are missing. Add `--format json` for a structured `{spending, savings, degraded}` payload.
